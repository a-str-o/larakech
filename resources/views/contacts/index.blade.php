...
@section('content')
    ...
    <div class="flex justify-between mb-4">
        <form action="{{ url('/') }}" method="GET" class="flex w-2/3" id="searchForm">
            <input type="text" name="search" id="search" placeholder="Search" 
                value="{{ $search ?? '' }}"
             class="border p-2 rounded w-full mr-4" oninput="debouncedSubmitSearchForm()">
        </form>
...
<script>
   let debounceTimer;
   function hideDuplicateContactAlert() {
        document.getElementById('duplicate-contact-alert').classList.add('hidden');
    }
    function debouncedSubmitSearchForm() {
